...
describe("Funcionalidad añadir un producto al carrito de compras", () => {
  ...
  it("TC7: Usuario añade un producto del PLP al Shopping-Cart exitosamente.", () => {
    ...
    inventoryPag.get.shoppingCardBadge()
      .should("exist")
      .should("have.text", "1")
      .click();
    cy.url().should("include", "cart");

    inventoryPag.get.inventoryItemName().should(
      "contain",
      products.name
    );
    inventoryPag.get.inventoryItemDesc().should(
      "contain",
      products.inventoryItemDescription
    );
    inventoryPag.get.inventoryItemPrice().should(
      "contain",
      products.inventoryItemPrice
    );
  });
